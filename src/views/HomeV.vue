...
const {
  elementX: card1X,
  elementY: card1Y,
  elementHeight: card1H,
  elementWidth: card1W,
  isOutside: card1S,
} = useMouseInElement(card1R);
const ct_card1 = computed(() => {
  return ct_construct(card1X, card1Y, card1H, card1W, card1S, 2);
});
const cl_card1 = computed(() => {
  return cl_construt(
    card1X,
    card1Y,
    card1H,
...
const {
  elementX: card2X,
  elementY: card2Y,
  elementHeight: card2H,
  elementWidth: card2W,
  isOutside: card2S,
} = useMouseInElement(card2R);
const ct_card2 = computed(() => {
  return ct_construct(card2X, card2Y, card2H, card2W, card2S, 2);
});
const cl_card2 = computed(() => {
  return cl_construt(
    card2X,
    card2Y,
    card2H,
...
const {
  elementX: card3X,
  elementY: card3Y,
  elementHeight: card3H,
  elementWidth: card3W,
  isOutside: card3S,
} = useMouseInElement(card3R);
const ct_card3 = computed(() => {
  return ct_construct(card3X, card3Y, card3H, card3W, card3S, 2);
});
const cl_card3 = computed(() => {
  return cl_construt(
    card3X,
    card3Y,
    card3H,
  ...
  <main class="home-container">
    ...
    <div id="service-home" class="home-services text-white">
      ...
      <div class="sc-container">
        <div class="sc-titles non-selectable">
          <div class="panimated sc-t-c" v-motion-slide-visible-once-left>
            <div class="text-italic sc-t-1 mc">Discover Our</div>
            <div class="sc-t-2 mc">SERVICES</div>
          </div>
        </div>

        <div
          class="panimated-card sc-card non-selectable"
          v-motion-fade-visible-once
        >
          <img
            src="../assets/home/service-cards/1.png"
            class="mc"
            ref="card1R"
            :style="{ transform: ct_card1, filter: cl_card1 }"
            alt="Card"
          />
          <img
            src="../assets/home/service-cards/2.png"
            class="mc"
            ref="card2R"
            :style="{ transform: ct_card2, filter: cl_card2 }"
            alt="Card"
          />
        </div>
        <div>
          <q-btn
            ...
            :ripple="false"
            @click="this.$router.push({ name: 'service' })"
          ></q-btn>
        </div>
      </div>
    </div>
    ...
    <div id="results-home" class="home-results text-white column">
      <div class="sc-container">
        ...
        <div class="sc-titles non-selectable">
          <div class="panimated sc-t-c" v-motion-slide-visible-once-left>
            <div class="text-italic sc-t-1 mc">See Our</div>
            <div class="sc-t-2 mc">RESULTS</div>
          </div>
        </div>
      </div>
      <div class="sc-container sc-container-2">
        <swiper
          :modules="swiperOptions.module"
          :mousewheel="true"
          ...
          loop
          :space-between="50"
          class="sc-swiper non-selectable"
          v-motion-fade-visible-once
        >
          <swiper-slide
            v-for="(item, i) in resultSrc"
            :key="i"
            :name="i + 1"
            @mouseover="hoveredS1 = i"
            @mouseleave="hoveredS1 = null"
            class="panimated-swipe"
          >
            <img
              :src="item.src"
              class="sc-items mc"
              :class="{
                dim: hoveredS1 !== null && hoveredS1 !== i && mobileCheck !== 1,
              }"
              alt="Card"
              :style="is"
            />
    ...
    <div
      id="doctors-home"
      class="home-doctors text-white"
      :style="{ background: 'url(' + require('@/assets/doctors.png') + ')' }"
    >
      <div class="sc-container testing" style="z-index: 3">
        <div class="sc-titles non-selectable">
          <div class="sc-t-c sc-t-c-2">
            <div class="lastResort panimated">
              <div class="text-italic sc-b-1 mc" v-motion-fade-visible-once>
                Meet Our
              </div>
              <div class="sc-b-2 mc" v-motion-fade-visible-once>DOCTORS</div>
            </div>
          </div>
        </div>
        <div class="panimated-card sc-card sc-card-2 non-selectable">
          <img
            ...
            :style="{ transform: ct_card3, filter: cl_card3 }"
            alt="Card"
          />
        </div>
      </div>
